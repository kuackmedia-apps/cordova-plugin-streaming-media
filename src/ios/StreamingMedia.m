...
@interface StreamingMedia() <AVPlayerViewControllerDelegate>
...
-(void)startPlayer:(NSString*)uri {
    ...
    // enable PiP and set delegate to restore UI
    if (@available(iOS 9.0, *)) {
        moviePlayer.allowsPictureInPicturePlayback = YES;
...
- (void)playerViewController:(AVPlayerViewController *)playerViewController restoreUserInterfaceForPictureInPictureStopWithCompletionHandler:(void (^)(BOOL restored))completionHandler {
    if (self.viewController.presentedViewController != playerViewController) {
